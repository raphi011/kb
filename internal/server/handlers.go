...
func (s *Server) renderFullPage(w http.ResponseWriter, r *http.Request, p views.LayoutParams) {
	w.Header().Add("Link", `<`+views.Asset("style.min.css")+`>; rel=preload; as=style`)
	w.Header().Add("Link", `<`+views.Asset("htmx.min.js")+`>; rel=preload; as=script`)
	w.Header().Add("Link", `<`+views.Asset("app.min.js")+`>; rel=preload; as=script`)
